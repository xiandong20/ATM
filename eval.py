...
from src.dataset.pix2pix_dataset import pix2pixDataset_val, create_val_dataset,SingleImageDataset
...
from src.utils.config import get_args,get_args_eval

# if __name__ == '__main__':
def eval(file_path):

    args = get_args_eval(file_path)

    # Preprocess the data for evaluating
    # dataset_val = pix2pixDataset_val(root_dir=args.val_data_dir)
    dataset_val = SingleImageDataset(root_dir=args.val_data_dir)
    ...
    for i, data in enumerate(data_loader_val):
        ...
        save_image(fake_image, save_path + 'predict')
        print("=======image", i + 1, "saved success=======")
    return save_path + "predict.jpg"