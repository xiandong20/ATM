...
def obs_data2modelarts(args):
    ...
    print("===>>>Files:", files)
    files = os.listdir(os.path.join(args.modelarts_data_dir , "facades/train"))      # /maps/train
    print("===>>>Train files:", files)
    files = os.listdir(os.path.join(args.modelarts_data_dir , "facades/test"))         # /maps/val
...
def modelarts_result2obs(args):
    ...
    mox.file.copy(src_url=os.path.join(args.modelarts_result_dir, 'Generator_200.ckpt'), dst_url=os.path.join(args.obs_result_dir, 'Pix2Pix_facades_200.ckpt'))  # Pix2Pix_maps
    mox.file.copy(src_url='Pix2Pix_facades.air', dst_url=os.path.join(args.obs_result_dir, 'Pix2Pix_facades.air'))  # Pix2Pix_maps
...
def export_AIR(args):
    ...
    # ckpt_url = args.ckpt
    # param_G = load_checkpoint(os.path.join(args.modelarts_result_dir, 'Generator_200.ckpt'))
    param_G = load_checkpoint(os.path.join(args.modelarts_result_dir, 'Generator_200.ckpt'))
    load_param_into_net(netG, param_G)

    input_shp = [args.batch_size, 3, 256, 256]
    input_array = Tensor(np.random.uniform(-1.0, 1.0, size=input_shp).astype(np.float32))
    target_shp = [args.batch_size, 3, 256, 256]
    ...
    export(train_net, *inputs, file_name="Pix2Pix_facades", file_format="AIR")
    ...
    for epoch in range(args.epoch_num):
        ...
        if (epoch+1) % 50 == 0:
            if args.run_distribute:
                ...
                save_checkpoint(netG, os.path.join(modelarts_result_dir, f"Generator_{epoch+1}.ckpt"))
            else:
                save_checkpoint(netG, os.path.join(modelarts_result_dir, f"Generator_{epoch+1}.ckpt"))
